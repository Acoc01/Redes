...
int main(int argc, char *argv[]) {
	...
	struct sockaddr_in server_address;//Se crea la estructura de la direccion del server
	memset(&server_address, 0, sizeof(server_address));//Se inicializa todo en 0
	server_address.sin_family = AF_INET;//Se especifica la familia de direccion AF_INET

	// htons: host to network short: transforms a value in host byte
	// ordering format to a short value in network byte ordering format
	server_address.sin_port = htons(SERVER_PORT);//Guarda el puerto en bytes en sin_port
	...
	struct sockaddr_in client_address; //Se crea un socket para la direccion del cliente
